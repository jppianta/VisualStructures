...
exports.includePackage = exports.setColors = exports.commonTags = exports.crossEnv = exports.open = exports.mkdirp = exports.ncp = exports.copy = exports.ifNotWindows = exports.ifWindows = exports.rimraf = exports.runInNewWindow = exports.series = exports.concurrent = undefined;
...
exports.includePackage = includePackage;
...
function includePackage(packageNameOrOptions) {
  var packageScriptsPath = typeof packageNameOrOptions === 'string' ? `./packages/${packageNameOrOptions}/package-scripts.js` : packageNameOrOptions.path;

  var startingDir = process.cwd().split('\\').join('/');

  var relativeDir = _path2.default.relative(startingDir, _path2.default.dirname(packageScriptsPath)).split('\\').join('/');

  var relativeReturn = _path2.default.relative(relativeDir, startingDir).split('\\').join('/');
  ...
  function replace(obj, prefix) {
    ...
    for (var key in obj) {
      if (key === 'description') {
        ...
        retObj[key] = series(`cd ${relativeDir}`, `npm start ${prefix}`, `cd "${relativeReturn}"`);
      } else if (typeof obj[key] === 'string') {
        retObj[key] = series(`cd ${relativeDir}`, `npm start ${prefix}${dot}${key}`);
      } else {
        retObj[key] = Object.assign({}, replace(obj[key], `${prefix}${dot}${key}`, `cd "${startingDir}"`));
      }
    }
    return retObj;
  }
